...
# -----------------------------
# Keep only integral data
# -----------------------------
df["value"] = pd.to_numeric(df["value"], errors="coerce")
data = df[df["measurement_type"] == "integral"].copy()

# -----------------------------
# Output folder
# -----------------------------
output_dir = Path.home() / "Desktop" / "IR_heatmaps_integrals_allbands"
...
for band in band_order:
    ...
    fig.suptitle(f"Integral heatmaps: {band} ({band_range} cm$^{{-1}}$)", fontsize=13)

    cbar = fig.colorbar(im, ax=axes, shrink=0.85)
    cbar.set_label("Integral area / a.u.")

    filename = output_dir / f"IR_integral_heatmap_{band}_{band_range.replace('-', '_')}.png"
    plt.savefig(filename, dpi=300, bbox_inches="tight")
    plt.show()

print(f"Heatmaps saved in:\n{output_dir}")